...
export async function createAttendanceSession({
  ...
  await pool.query(
    `INSERT INTO attendance_sessions 
      (session_id, teacher_id, subject, year, division, stream, started_at, status) 
     VALUES (?, ?, ?, ?, ?, ?, NOW(), 'active')`,
    [sessionId, teacherId, subject, year, division, stream]
  );
...
export async function finalizeAttendanceSession(
  ...
  try {
    ...
    const values = attendanceRecords.map(() => '(?, ?, ?, ?, NOW())').join(',');
...
export async function getTeacherStats(teacherId) {
  ...
  const average = total ? Math.round(((summary.total_present || 0) / total) * 100) : 0;
